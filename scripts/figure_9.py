...
#    -----------------------------
#    python figure_9.py -i "[../data/forecast/forecast_performance_EEE.nc, ../data/forecast/forecast_performance_expert.nc]" -b "[Passes Dangereuses, Peribonka, Lac Manouane, Montagnes Blanches] -p figure_9.pdf -t pdf
...
usetex           = False
serif            = False
only_successful  = False       # only successful calibrated days
infiles          = ''
...
alphamod    = 0.9      # 1.0-no transparency, 0.0-full transparency
...
selected_metrics = ['waeT-7_0', 'VE_T1_3', 'VE_W01','VE_W02','VE_W04','VE_W08','VE_W12','VE_W16','VE_W20','VE_W26','VE_W52']


success_rate_JulToNov_expert = np.ones([len(selected_metrics), len(basin_name), len(recipe_expert)]) * -9999.0
for iiobj,iobj in enumerate(objective_info):

    if iobj in selected_metrics:
        ...
        for iibasin, ibasin in enumerate(basin_name):
            ...
            for iirecipe, irecipe in enumerate(recipe_expert):
                ...
                # -------------------------
                # get success rate for Jul to Nov
                # -------------------------
                weeks             = np.array([ objective_info[oo][1] for oo in objective_info ])
                norm_obj_openloop = np.abs(objective_open_loop_expert[iirecipe][iibasin]/weeks)       # taking abs() because we are not interested if we over- or underestimate the VE
                norm_obj          = np.abs(objective_expert[iirecipe][iibasin]/weeks)                 # taking abs() because we are not interested if we over- or underestimate the VE
                
                tt_idx=((np.array([dd.month if dd != -9999 else -9999 for dd in simtime_expert[iirecipe][iibasin].data ]) >= 7) &
                        (np.array([dd.month if dd != -9999 else  9999 for dd in simtime_expert[iirecipe][iibasin].data ]) <= 11))
                if only_successful:
                    idx_success_calib = np.where(objective_expert[iirecipe][iibasin][tt_idx,2] < 0.05)[0]    # find where "relT0" is smaller than 5%
                else:
                    idx_success_calib = np.arange(np.shape(objective_expert[iirecipe][iibasin][tt_idx,2])[0])

                mask_normobj = np.ones(np.shape(norm_obj[tt_idx,iiobj]),dtype=bool)                       # All true
                
                # count how often we are better than open loop
                perc_success = np.sum((norm_obj[tt_idx,iiobj] < norm_obj_openloop[tt_idx,iiobj])[idx_success_calib])*1.0 / np.sum((mask_normobj)[idx_success_calib]) * 100.0
                # print( ibasin,": ",irecipe,": ",iobj,": percentage success of FC obver OL: ",perc_success,'% (Apr-Jun period)')

                success_rate_JulToNov_expert[selected_metrics.index(iobj), iibasin, iirecipe] = perc_success




success_rate_JulToNov_EEE = np.ones([len(selected_metrics), len(basin_name), len(recipe_EEE)]) * -9999.0
for iiobj,iobj in enumerate(objective_info):
    ...
    if iobj in selected_metrics:
        ...
        for iibasin, ibasin in enumerate(basin_name):
            ...
            for iirecipe, irecipe in enumerate(recipe_EEE):
                ...
                # -------------------------
                weeks             = np.array([ objective_info[oo][1] for oo in objective_info ])
                norm_obj_openloop = np.abs(objective_open_loop_EEE[iirecipe][iibasin]/weeks)       # taking abs() because we are not interested if we over- or understeimate the VE
                norm_obj          = np.abs(objective_EEE[iirecipe][iibasin]/weeks)                 # taking abs() because we are not interested if we over- or understeimate the VE
                
                tt_idx=((np.array([dd.month if dd != -9999 else -9999 for dd in simtime_EEE[iirecipe][iibasin].data ]) >= 7) &
                        (np.array([dd.month if dd != -9999 else  9999 for dd in simtime_EEE[iirecipe][iibasin].data ]) <= 11))

                if only_successful:
                    idx_success_calib = np.where(objective_EEE[iirecipe][iibasin][tt_idx,2] < 0.05)[0]    # find where "relT0" is smaller than 5%
                else:
                    idx_success_calib = np.arange(np.shape(objective_expert[iirecipe][iibasin][tt_idx,2])[0])
                ...
                success_rate_JulToNov_EEE[selected_metrics.index(iobj), iibasin, iirecipe] = perc_success
...
# ----------------------------------------------------------------
# success rate of OL (when calibration was happening) for Jul to Nov
# ----------------------------------------------------------------
...
basin_sel = [[0,2,1,3]] # [[0],[0,2,1,3]]
irecipe   = recipe[0]
iirecipe  = recipe.index(irecipe)
...
for iibasin,ibasin in enumerate(basin_name):
    ...
    for iirecipe_EEE, irecipe_EEE in enumerate(recipe_EEE):
        ...
        sub.plot(range(len(selected_metrics)),success_rate_JulToNov_EEE[:,iibasin,iirecipe_EEE],
                     color=color_method)

    # Expert recipe
    for iirecipe_expert, irecipe_expert in enumerate(recipe_expert):
        color_method   = colors[nrecipe_EEE+iirecipe_expert+1]
        sub.plot(range(len(selected_metrics)),success_rate_JulToNov_expert[:,iibasin,iirecipe_expert],
                     color=color_method)
    ...
    ntime = np.shape(simtime_EEE[iirecipe][iibasin])[0]
    line2 = sub.plot(range(len(selected_metrics)),np.ones(len(selected_metrics))*50.0)
    plt.setp(line2, linestyle='--', linewidth=lwidth, marker=None, color='gray')
    if (iplot == 2): # first plot
        #
        # values below 50%
        sub.text(0.0, 49.0, 'worse than OL', color='gray', fontsize='x-small', va='top', ha='left',rotation=0)
        # sub.text(range(len(selected_metrics))[-1], 49.0, 'worse than OL', color='gray', fontsize='x-small', va='top', ha='right',rotation=0)
        #
        # values above 50%
        sub.text(0.0, 51.0, 'better than OL', color='gray', fontsize='x-small', va='bottom', ha='left',rotation=0)
        # sub.text(range(len(selected_metrics))[-1], 51.0, 'better than OL', color='gray', fontsize='x-small', va='bottom', ha='right',rotation=0)
    ...
    plt.setp(sub, xlim=[xmin,xmax], ylim=[0,100])
