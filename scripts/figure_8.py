...
#             grp_var = opt_results_EEE.createVariable(irecipe+'/'+ibasin+'/simtime_EEE'             , "f4",("ntime"),         zlib=True)
...
#             opt_results_EEE.groups[irecipe].groups[ibasin].variables['simtime_EEE'            ][:]    = np.ma.array([(isimtime_EEE[ii]-refdate).days if not(isimtime_EEE.mask[ii]) else -9999 for ii in np.arange(nfiles)])           
...
#             grp_var = opt_results_expert.createVariable(irecipe+'/'+ibasin+'/simtime_expert'             , "f4",("ntime"),         zlib=True)
#             grp_var.setncattr("long_name",     "time")
#             grp_var.setncattr("units",         "days since "+str(refdate))
#             grp_var.setncattr("calendar",      "gregorian")
#             grp_var.setncattr("standard_name", "time")
...
#             opt_results_expert.groups[irecipe].groups[ibasin].variables['simtime_expert'            ][:]    = np.array([(isimtime_expert[ii]-refdate).days if not(isimtime_expert.mask[ii]) else -9999 for ii in np.arange(nfiles)])    
